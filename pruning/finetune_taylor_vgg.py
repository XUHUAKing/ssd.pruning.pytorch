...
class FilterPrunner:
	...
	# this function will be called when gradients are ready above
	def compute_rank(self, grad):
		activation_index = len(self.activations) - self.grad_index - 1
		activation = self.activations[activation_index]
		# calculate Theta_TE(h_i), the ranking criterion based on Taylor expansion
		# activate = (N, C, W, H) where C is the channel number,
		# so dim=1 is for individual feature map, corresponding to a filter
		values = \
			torch.sum((activation * grad), dim = 0).\
				sum(dim=2).sum(dim=3)[0, :, 0, 0].data # |delta_C/delta_h_i * h_i|
		...
		if activation_index not in self.filter_ranks:
			self.filter_ranks[activation_index] = \
				torch.FloatTensor(activation.size(1)).zero_().cuda() # size(1) represents the num of filter/individual feature map
	...
	def lowest_ranking_filters(self, num):
		...
		# return a list with num smallest elements from the data define by item 2 (i.e. self.filter_ranks[][])
		return nsmallest(num, data, itemgetter(2))

	# "raw values": scale varies with the depth of the parameter's layer in the network
	# perform layer-wise l2-normalization to rescale across layers
	...
	def get_prunning_plan(self, num_filters_to_prune):
		...
		filters_to_prune_per_layer = {} # (layer - filter indices)
		...
		for l in filters_to_prune_per_layer:
			# filters_to_prune_per_layer[l] = sorted(filters_to_prune_per_layer[l], reverse = True)
			filters_to_prune_per_layer[l] = sorted(filters_to_prune_per_layer[l])
			for i in range(len(filters_to_prune_per_layer[l])):
				filters_to_prune_per_layer[l][i] = filters_to_prune_per_layer[l][i] - i # from biggest to smallest
