...
from peleebase import PeleeBasicModel 
...
model_names.append('peleebasicmodel')
...
def main():
	global args, best_prec1, filename, prefilename
	...
	if args.pretrained:
		...
		if args.arch.startswith('mobilenetv1'):
			...
			model = MobileNetV2()
		elif args.arch.startswith('peleebasicmodel'):
			model = PeleeBasicModel()
		else:
	...
	if args.evaluate:
		if os.path.isfile(args.evaluate):
			...
			#checkpoint = torch.load(args.evaluate)
			#state_dict = torch.load(args.evaluate)
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		filename = "./mobilenet_models/" + str(epoch) + "_mobilenetv2_top1_"+str(round(prec1.cpu().data.tolist(), 4))+".pth.tar"
		save_checkpoint({
			'epoch': epoch + 1,
            		'arch': args.arch,
...
def train(train_loader, model, criterion, optimizer, epoch):
	...
	for i, (input, target) in enumerate(train_loader):
		...
		if i % args.print_freq == 0:
			...
			print(time_stamp.strftime('I%m%d %H:%M:%S'), end = '\t')
			print('Epoch: [{0}][{1}/{2}]\t'
...
def save_checkpoint(state, is_best, prefilename, filename):
	torch.save(state, filename)
	if is_best:
		if prefilename != "":
			os.remove(prefilename)
